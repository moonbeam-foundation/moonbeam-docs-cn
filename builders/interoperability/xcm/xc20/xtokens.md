...
4. 将`destWeightLimit`设置为`Unlimited`。在JavaScript中，您需要将`Unlimited`设置为`null`（如[`XcmV3WeightLimit`的TypeScript接口](https://github.com/moonbeam-foundation/moonbeam/blob/{{ networks.moonbase.spec_version }}/typescript-api/src/moonbase/interfaces/augment-api-tx.ts#){target=_blank}中所述）：
...
3. 将目的地权重限制设置为`Unlimited`。在JavaScript中，您需要将`Unlimited`设置为`null`（如[`XcmV3WeightLimit`的TypeScript接口](https://github.com/moonbeam-foundation/moonbeam/blob/{{ networks.moonbase.spec_version }}/typescript-api/src/moonbase/interfaces/augment-api-tx.ts#){target=_blank}中所述：
