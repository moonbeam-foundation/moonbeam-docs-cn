...
为了将这些XCM消息发送到中继链，*通常会调用*[Polkadot XCM Pallet](https://github.com/paritytech/polkadot/tree/master/xcm/pallet-xcm){target=_blank}。Moonbeam还有一个[XCM Transactor Pallet](/builders/interoperability/xcm/xcm-transactor){target=_blank}，这将流程简化为一个抽象XCM消息传递构造函数的调用。
