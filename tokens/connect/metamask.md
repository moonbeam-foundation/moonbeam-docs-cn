...
开发人员可以利用Moonbeam与以太坊兼容的特色，将一些如[MetaMask](https://metamask.io/){target=_blank}的工具整合至DApp中。如此一来，就可以使用MetaMask提供的库与要部署的链相交互。
...
[MetaMask](https://metamask.io){target=_blank}安装完毕后，将会跳出一个欢迎页面。点击**Get Started**进行设置过程。
...
当您完成安装[MetaMask](https://metamask.io/){target=_blank}，并创建或导入账户后，您可以通过点击右上角的头像打开网络设置后，并点击**Add Network**，将其连接至Moonbeam。
...
您也可以尝试使用MetaMask发送一些Token。在本示例中，您将需要两个账户。为此，您需要再创建一个新的账户。两个账户准备完毕后，点击**Send**开启一笔转账。选择**Transfer between my accounts**选项，您可以尝试交易100个Token并保持其他设置不变：
