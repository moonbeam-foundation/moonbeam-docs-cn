...
如同先前提到的，波卡目前对所有XCM指令采取[固定权重数量](https://github.com/paritytech/polkadot/blob/e76cd144f9dad8c1304fd1476f92495bbb9ad22e/runtime/polkadot/src/xcm_config.rs#L95){target=_blank}的计算方式，也就是`{{ networks.polkadot.xcm_instructions.weight.display }}`权重单位。
...
在波卡中，[`ExtrinsicBaseWeight`](https://github.com/paritytech/polkadot/blob/v0.9.36/runtime/polkadot/constants/src/weights/extrinsic_weights.rs#L55){target=_blank}被设置为`{{ networks.polkadot.extrinsic_base_weight.display }}`，也就是[一分的十分之一](https://github.com/paritytech/polkadot/blob/v0.9.36/runtime/polkadot/constants/src/lib.rs#L88){targer=blank}。一分为`10^10 / 100`。
