...
--8<-- 'text/xcm/general-xcm-definitions2.md'
 ...
 - **Derivative account** —— 使用简单的索引从另一个账户衍生的账户。此操作通过计算`modlpy/utilisuba` + `originalAddress` + `index`的Blake2哈希所获得。由于该账户的私钥是未知的，所以必须通过`utility.asDerivative`方法发起交易。当通过衍生账户进行交易时，交易费用由起始账户支付，而交易是由衍生账户发送。举例而言，Alice有一个索引为`0`的衍生账户，如果她要使用`asDerivative`函数转移资产，则她仍需支付交易费用，而转移的资金将从索引为`0`的衍生账户中提取。您可以[使用脚本计算衍生账户](https://github.com/albertov19/PolkaTools/blob/main/calculateDerivedAddress.ts){target=_blank}
